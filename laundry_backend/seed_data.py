...
def create_orders():
    ...
    for client in clients:
        # Ensure client has a profile; if not, create one with defaults
        profile, created = ClientProfile.objects.get_or_create(user=client)
        if created:
            print(f"   ⚠️ Created missing profile for {client.phone_number}")
        ...
        for j in range(num_orders):
            ...
            if random.random() < 0.4:
                total_price += Decimal(random.randint(0, 500))
