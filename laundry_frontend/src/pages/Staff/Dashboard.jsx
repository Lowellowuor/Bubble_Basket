  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="animate-pulse text-brand-green">Loading orders...</div>
      </div>
    )
  }
    ...
    <div className="animate-fadeIn">
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="card p-2 text-center">
          ...
          <p className="text-lg font-brand text-green-600">{paidOrders}</p>
        </div>
        <div className="card p-2 text-center">
          <span className="text-xs text-text-secondary">Pending</span>
          <p className="text-lg font-brand text-yellow-600">{pendingOrders}</p>
        </div>
        <div className="card p-2 text-center">
          <span className="text-xs text-text-secondary">Failed</span>
          <p className="text-lg font-brand text-red-600">{failedOrders}</p>
        </div>
      </div>
          ...
          return (
            <div key={key} className="card p-3 flex flex-col min-h-[300px]">
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-semibold text-sm text-brand-green uppercase tracking-wider flex items-center gap-2">
                ...
                {columnOrders.map((order) => (
                  <div key={order.id} className="bg-gray-50 rounded-lg p-3 text-sm border border-gray-100 hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="font-brand text-brand-green font-semibold text-xs uppercase tracking-wider">
                          {order.order_number}
                        </p>
                      ...
                      {(key === 'created' || key === 'picked_up' || key === 'ready') && (
                        <div className="flex items-center gap-0.5 ml-auto">
                          ...
                          <select
                            ...
                            className="text-[10px] border border-gray-200 rounded-full px-1.5 py-0.5 bg-white focus:outline-none focus:ring-1 focus:ring-brand-green"
                          >
